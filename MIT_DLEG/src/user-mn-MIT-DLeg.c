...
void MIT_DLeg_fsm_1(void)
{
	...
	switch(fsm1State)
	{
		...
		case STATE_TASK_MACHINE:
			...
			rigid1.mn.genVar[1] = (int16_t) (100.0*get_learner()->sum_sigma[72]); //
			rigid1.mn.genVar[2] = (int16_t) (100.0*get_learner()->sum_sigma[144]); //
			rigid1.mn.genVar[3] = (int16_t) (100.0*get_learner()->sum_sigma[216]); //
			rigid1.mn.genVar[4] = (int16_t) (100.0*get_learner()->mu[68]); //
			rigid1.mn.genVar[5] = (int16_t) (100.0*get_learner()->mu[69]); //
			rigid1.mn.genVar[6] = (int16_t) (100.0*get_task_machine()->torque_raw); //
			rigid1.mn.genVar[7] = (int16_t) (100.0*get_task_machine()->angle_raw); //
			rigid1.mn.genVar[8] = (int16_t) (100.0*get_learner()->mu[70]);//
			rigid1.mn.genVar[9] = (int16_t) (100.0*get_learner()->mu[71]);//
			iter++;
			if (iter == 30000)
				iter = 0;
				break;
